...
class CalendarService
{
     ...
     * @return array
     */
    public function getWeeks($events,$eventstart)
    {
        $weeks = [];
        $week = '';
        ...
        for ($day = 1; $day <= $days_in_month; $day++, $day_of_week++) {
            ...
            if (Carbon::now()->format('Y-m-d') === $date) {
                $week .= '<td class="today">' . $day.' today!';
            } elseif(in_array($date, $eventstart)){
               foreach($events as $event){
                if($event['eventdate_st']===$date){
                    if ($header==0) {
                        $week .= '<td class="event">'.$day;
                        $header = 1;
                    }
                    $week .= '<a href="/groupEventMenu/'.$event['id'].'">'.'「'.$event['event_name'].'」'.'開始日'.'・'.$event['eventdate_ed'].'まで'.'<br>';
                }
               }
            } else {
                $week .= '<td>' . $day;
